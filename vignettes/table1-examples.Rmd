...
[knitr](https://yihui.name/knitr/) and [R
...
across strata.  There is currently no facility built into the package to
accomplish this, but with the use of a custom render function and an empty
stratum, it can be "tricked" into producing the desired result.

The following example uses the `lalonde` data from the `MatchIt` package. In
this case, a chi-square test of independence is used for categorical variables,
and a t-test for continuous variables (other tests could be used if desired,
...
Admittedly, this is not very elegant, and is definitely a hack. But it is still
a neat illustration of the flexibility of the package in terms of being able to
accomplish something that we never really planned for.
